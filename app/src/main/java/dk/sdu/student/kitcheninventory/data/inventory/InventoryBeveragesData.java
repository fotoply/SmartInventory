...
public class InventoryBeveragesData
{
    ...
    {
         return Inventory.getInstance().getCompartmentByName("Drikkelse").getProducts().stream().map(product -> new Ingredient(product, product.getAmount())).collect(Collectors.toList());
    }
}
