...
public class InventoryFruitsData
{
    public static List<Ingredient> GetData()
    {
        return Inventory.getInstance().getCompartmentByName("Frugt").getProducts().stream().map(product -> new Ingredient(product, product.getAmount())).collect(Collectors.toList());
    }
}
